...
// In-memory cache for discovered models keyed by API key signature
let modelDiscoveryCache: { keySig: string; apiVersion: string; models: string[] } | null = null;
 ...
 */
async function discoverSupportedModels(apiKey: string): Promise<{ apiVersion: string; models: string[] }> {
  const keySig = apiKey.slice(-8);
  if (modelDiscoveryCache && modelDiscoveryCache.keySig === keySig && modelDiscoveryCache.models.length > 0) {
      ...
      if (res.ok) {
        ...
        if (data.models && Array.isArray(data.models)) {
          ...
          if (usable.length > 0) {
            ...
            modelDiscoveryCache = { keySig, apiVersion: ver, models: usable };
            return modelDiscoveryCache;
          }
        }
      }
  ...
  // Fallback defaults if probe timed out or failed
  return {
    apiVersion: 'v1beta',
...
function formatParsedEquation(parsed: any): ColorizedEquation {
  return {
    id: `eq-${Date.now()}`,
...
/**
 * Calls Gemini with dynamic model discovery and automatic fallback.
 */
export async function deconstructWithGemini(
  ...
  const { apiVersion, models } = await discoverSupportedModels(apiKey.trim());
  ...
  for (const model of modelsToTry) {
    try {
      ...
      // If 400 mentions schema, retry once without responseSchema
        ...
        if (errorBody.includes('schema') || errorBody.includes('responseSchema')) {
          ...
          if (fallbackResp.ok) {
            ...
            if (rawJson) {
              ...
              return formatParsedEquation(parsed);
            }
          }
        }
      ...
      // Prioritize this working model for future calls
      if (modelDiscoveryCache) {
        modelDiscoveryCache.models = [model, ...modelDiscoveryCache.models.filter(m => m !== model)];
      }
